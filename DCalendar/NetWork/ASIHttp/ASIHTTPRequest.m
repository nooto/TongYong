...
//    self.tag = [sharedQueue operationCount];
}
...
- (void)requestFinished
{
	...
	if ([self isPACFileRequest]) {
		...
		//shizejing [self performSelectorOnMainThread:@selector(reportFinished) withObject:nil waitUntilDone:[NSThread isMainThread]];
        [self reportFinished];
	}
}
 ...
 */
    if(_didRequestFinishedBlock){
        _didRequestFinishedBlock([self responseData],nil);
    }
}
...
/* shizejing
    ...
    if(_didRequestFinishedBlock){
//        NSString *err = [NSString stringWithFormat:@"%@",self.error];
//        NSData *data = [err dataUsingEncoding:NSUTF8StringEncoding];
        _didRequestFinishedBlock(nil,self.error);
    }
}
...
- (void)failWithError:(NSError *)theError
{
	...
	if ([self isPACFileRequest]) {
		...
		//shizejing [failedRequest performSelectorOnMainThread:@selector(reportFailure) withObject:nil waitUntilDone:[NSThread isMainThread]];
        [self reportFailure];
	}
...
//	isBandwidthThrottled = [ASIHTTPRequest isNetworkReachableViaWWAN];
    isBandwidthThrottled = (ReachableViaWWAN == status);
	[bandwidthThrottlingLock unlock];
    
    //add by shizejing 2014-11-6
    if(status == ReachableViaWiFi)
    {
        NSLog(@"Server is reachable via Wifi");
        [sharedQueue setMaxConcurrentOperationCount:6];
        //        [self checkAndRestoreFrozenOperations];
    }
    else if(status == ReachableViaWWAN)
    {
        if(shouldThrottleBandwidthForWWANOnly) {
            NSLog(@"reachable only via cellular data");
            [sharedQueue setMaxConcurrentOperationCount:0];
        } else {
             NSLog(@"reachable only via cellular data");
