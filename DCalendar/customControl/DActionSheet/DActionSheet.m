...
#define ACTIONSHEET_BACKGROUNDCOLOR             [UIColor colorWithRed:217/255.00f green:217/255.00f blue:217/255.00f alpha:1]
#define CANCEL_BUTTON_COLOR                     [UIColor colorWithRed:163/255.00f green:163/255.00f blue:163/255.00f alpha:1]
...
@property (nonatomic,assign) CGFloat DActionSheetHeight;
@property (nonatomic,assign) id<DActionSheetDelegate>delegate;
...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle destructionButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitlesArray
{
    ...
    if (title) {
        ...
        self.DActionSheetHeight = self.DActionSheetHeight + 2*TITLE_INTERVAL_HEIGHT+TITLE_HEIGHT;
    ...
    if (destructiveButtonTitle) {
        ...
        if (self.isHadTitle == YES) {
            //当有title时
            [destructiveButton setFrame:CGRectMake(destructiveButton.frame.origin.x, self.DActionSheetHeight, destructiveButton.frame.size.width, destructiveButton.frame.size.height)];
            
            if (otherButtonTitlesArray && otherButtonTitlesArray.count > 0) {
                self.DActionSheetHeight = self.DActionSheetHeight + destructiveButton.frame.size.height+BUTTON_INTERVAL_HEIGHT/2;
            }
            else{
                self.DActionSheetHeight = self.DActionSheetHeight + destructiveButton.frame.size.height+BUTTON_INTERVAL_HEIGHT;
            }
        }
        else{
            //当无title时
            if (otherButtonTitlesArray && otherButtonTitlesArray.count > 0) {
                self.DActionSheetHeight = self.DActionSheetHeight + destructiveButton.frame.size.height+(BUTTON_INTERVAL_HEIGHT+(BUTTON_INTERVAL_HEIGHT/2));
            }
            else{
                self.DActionSheetHeight = self.DActionSheetHeight + destructiveButton.frame.size.height+(2*BUTTON_INTERVAL_HEIGHT);
            }
        }
        ...
        if (otherButtonTitlesArray.count > 0) {
            ...
            if (self.isHadTitle == NO && self.isHadDestructionButton == NO) {
                for (int i = 0; i<otherButtonTitlesArray.count; i++) {
                    ...
                    if (i != otherButtonTitlesArray.count - 1) {
                        self.DActionSheetHeight = self.DActionSheetHeight + otherButton.frame.size.height+(BUTTON_INTERVAL_HEIGHT/2);
                    }
                    else{
                        self.DActionSheetHeight = self.DActionSheetHeight + otherButton.frame.size.height+(2*BUTTON_INTERVAL_HEIGHT);
                    }
            ...
            if (self.isHadTitle == YES || self.isHadDestructionButton == YES) {
                for (int i = 0; i<otherButtonTitlesArray.count; i++) {
                    ...
                    otherButton.tag = self.postionIndexNumber;
                    [otherButton addTarget:self action:@selector(clickOnButtonWith:) forControlEvents:UIControlEventTouchUpInside];
                    [otherButton setFrame:CGRectMake(otherButton.frame.origin.x, self.DActionSheetHeight, otherButton.frame.size.width, otherButton.frame.size.height)];
                    
                    if (i != otherButtonTitlesArray.count - 1) {
                        self.DActionSheetHeight = self.DActionSheetHeight + otherButton.frame.size.height+(BUTTON_INTERVAL_HEIGHT/2);
                    }
                    else{
                        self.DActionSheetHeight = self.DActionSheetHeight + otherButton.frame.size.height+(BUTTON_INTERVAL_HEIGHT);
                    }
    ...
    if (cancelButtonTitle) {
        ...
        if (self.isHadTitle == NO && self.isHadDestructionButton == NO && self.isHadOtherButton == NO) {
             self.DActionSheetHeight = self.DActionSheetHeight + cancelButton.frame.size.height+(2*BUTTON_INTERVAL_HEIGHT);
        }
        
        //当有title或destructionButton或otherbuttons时
        if (self.isHadTitle == YES || self.isHadDestructionButton == YES || self.isHadOtherButton == YES) {
            [cancelButton setFrame:CGRectMake(cancelButton.frame.origin.x, self.DActionSheetHeight, cancelButton.frame.size.width, cancelButton.frame.size.height)];
            self.DActionSheetHeight = self.DActionSheetHeight + cancelButton.frame.size.height+BUTTON_INTERVAL_HEIGHT;
        }
...
//        [self.backGroundView setFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height-self.DActionSheetHeight, [UIScreen mainScreen].bounds.size.width, self.DActionSheetHeight)];
...
//    otherButton.layer.borderColor = BUTTON_BORDER_COLOR;
    ...
    [otherButton setTitleColor:[Utility colorFromColorString:@"#555555"] forState:UIControlStateNormal];
    [otherButton setTitleColor:[Utility colorFromColorString:@"#555555"] forState:UIControlStateHighlighted];
    return otherButton;
}
...
- (void)tappedCancel
{
    if ([self.delegate respondsToSelector:@selector(didClickHidden)] == YES) {
        [self.delegate didClickHidden];
    }
//    [UIView animateWithDuration:ANIMATE_DURATION animations:^{
...
//    } completion:^(BOOL finished) {
//        if (finished) {
////            [self.backGroundView setFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, ScreenHeight)];
//            self.alpha = 1.0f;
