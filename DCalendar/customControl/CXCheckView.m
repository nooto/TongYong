...
#import "CXCheckView.h"

@interface CXCheckView ()

@property (nonatomic, strong) NSMutableArray *arrButtonTitles;
@property (nonatomic, weak) UIButton *mCheckBtn;
@property (nonatomic, weak) UIImageView *mCheckImage;

@end

@implementation CXCheckView
...
- (id) initWithMessage:(NSString *)message delegate:(id)delegate buttonTitles:(NSString *)firstObject, ... NS_REQUIRES_NIL_TERMINATION{
    ...
    if (self) {
        ...
        UIView *bkview = [[UIView alloc] initWithFrame:CGRectMake(10, 205, 300, 180)];
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(20, 40, bkview.frame.size.width-48, height)];
...
//        label.textAlignment = NSTextAlignmentCenter;
        ...
        [bkview addSubview:check];
        _mCheckBtn = check;
        check.frame = CGRectMake(20, 83, 136, 35);
//        check.backgroundColor = [Utility colorFromColorString:@"#CECECE"];
//        [check setTitle:[_arrButtonTitles firstObject] forState:UIControlStateNormal];
        [check addTarget:self action:@selector(checkBtnPressed:) forControlEvents:UIControlEventTouchDown];
//        [check addTarget:self action:@selector(cancelButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        [check setTitle:@"仅本期" forState:UIControlStateNormal];
//        check.backgroundColor = [Utility colorFromColorString:@"#CECECE"];
        [check setTitleColor:[Utility colorFromColorString:@"#333333"] forState:UIControlStateNormal];
        check.layer.cornerRadius = 3;
        check.layer.masksToBounds = YES;
        check.tag = 0;
        check.titleLabel.font = [UIFont systemFontOfSize:16];
        
        UILabel *labelext = [[UILabel alloc] initWithFrame:CGRectMake(120, 82, 200, 35)];
        [bkview addSubview:labelext];
        labelext.text = @"（取消后即开始下一期）";
        labelext.textColor = [Utility colorFromColorString:@"#a1a1a1"];
        labelext.font = check.titleLabel.font;
//        labelext.textAlignment = NSTextAlignmentRight;
        
        UIImageView *checkimage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 7, 25, 25)];
        checkimage.image = [UIImage imageNamed:@"fee_payed.png"];
        [check addSubview:checkimage];
        _mCheckImage = checkimage;
//        [check setImage: forState:UIControlStateNormal];
//        weakSelf.mCheckBoxView.image = [UIImage imageNamed:@"fee_payed.png"];
//        weakSelf.mCheckBoxView.image = [UIImage imageNamed:@"fee_nopay.png"];
//        check.imageEdgeInsets = UIEdgeInsetsMake(10, 20, 10, 50);
        //        cancel.titleEdgeInsets = UIEdgeInsetsMake(10, 10, 10, 30);
        
        
        UIButton *cancel = [UIButton buttonWithType:UIButtonTypeCustom];
        [bkview addSubview:cancel];
        cancel.frame = CGRectMake(9, 126, 136, 47);
        ...
        ok.frame = CGRectMake(155, 126, 136, 47);
...
- (void)checkBtnPressed:(UIButton *)btn{
    btn.tag = !btn.tag;
    if (!btn.tag) {
        _mCheckImage.image = [UIImage imageNamed:@"fee_payed.png"];
    }
    else{
        _mCheckImage.image = [UIImage imageNamed:@"fee_nopay.png"];
    }
}
...
- (void)cancelButtonPressed:(UIButton *)btn{
    ...
    if ([_delegate respondsToSelector:@selector(alertView:clickedButtonAtIndex:check:)]) {
        [_delegate alertView:self clickedButtonAtIndex:btn.tag check:_mCheckBtn.tag];
    }
