...
@interface DDatabasePool()

- (void)pushDatabaseBackInPool:(DDatabase*)db;
- (DDatabase*)db;

@end


@implementation DDatabasePool
...
- (id)initWithPath:(NSString*)aPath {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"DDB.%@", self] UTF8String], NULL);
        _databaseInPool     = DDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = DDBReturnRetained([NSMutableArray array]);
    }
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    DDBRelease(_path);
    DDBRelease(_databaseInPool);
    DDBRelease(_databaseOutPool);
...
- (void)pushDatabaseBackInPool:(DDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The DDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (DDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [DDatabase databaseWithPath:_path];
        }
