...
@interface DResultSet (Private)
- (NSMutableDictionary *)columnNameToIndexMap;
- (void)setColumnNameToIndexMap:(NSMutableDictionary *)value;
@end

@implementation DResultSet
@synthesize query=_query;
@synthesize columnNameToIndexMap=_columnNameToIndexMap;
@synthesize statement=_statement;

+ (id)resultSetWithStatement:(DStatement *)statement usingParentDatabase:(DDatabase*)aDB {
    
    DResultSet *rs = [[DResultSet alloc] init];
    
    [rs setStatement:statement];
    [rs setParentDB:aDB];
    
    return DDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    DDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return DDBReturnAutoreleased([dict copy]);
    }
    else {
...
- (void)setParentDB:(DDatabase *)newDb {
    _parentDB = newDb;
}
