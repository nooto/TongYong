...
// avatar
- (void)setAvatarNormalImage:(UIImage *)avatarImage HightImage:(UIImage*)hightImage{
    if (avatarImage) {
...
// set info
- (void)setFirstLineTextLabel:(NSString *)first SecondLabel:(NSString*)secondText{
	if (first) {
		self.firstLineTextLabel.text = first;
	}
	
	if (secondText) {
		self.secondLineLabel.text = secondText;
	}
}
...
//    self.parallaxHeight = 170;
    ...
    CGFloat waterDropRefreshHeight = 120;
    CGFloat waterDropRefreshWidth = 60;
    _waterDropRefresh = [[XHWaterDropRefresh alloc] initWithFrame:CGRectMake(46, CGRectGetHeight(self.bounds) - waterDropRefreshHeight, waterDropRefreshWidth, waterDropRefreshHeight)];
    _waterDropRefresh.refreshCircleImage = [UIImage imageNamed:@"circle"];
    _waterDropRefresh.offsetHeight = 40; // 线条的长度
    [self addSubview:self.waterDropRefresh];
    
    self.showUserInfoViewOffsetHeight = 66 + 10;//(10 + 44 + 45) * (320/480);
    ...
    CGSize avatarButtonSize = CGSizeMake(46.5, 46.5);
    CGFloat linePositionX = 60.f;
    _avatarButton = [[UIButton alloc] initWithFrame:CGRectMake(linePositionX - avatarButtonSize.width/2, 0, avatarButtonSize.width, avatarButtonSize.height)];
	[_avatarButton addTarget:self action:@selector(avatarButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    _firstLineTextLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_avatarButton.frame), CGRectGetMaxY(_avatarButton.frame), 207, 18)];
    _firstLineTextLabel.textColor = [UIColor whiteColor];
    _firstLineTextLabel.backgroundColor = [UIColor clearColor];
    _firstLineTextLabel.shadowColor = [UIColor blackColor];
    _firstLineTextLabel.shadowOffset = CGSizeMake(0, 2);
    _firstLineTextLabel.font = [UIFont boldSystemFontOfSize:16.0f];
    
    
    _secondLineLabel = [[UILabel alloc] initWithFrame:CGRectMake(_firstLineTextLabel.frame.origin.x, CGRectGetMaxY(_firstLineTextLabel.frame), 207, 18)];
    _secondLineLabel.textColor = [UIColor whiteColor];
    _secondLineLabel.backgroundColor = [UIColor clearColor];
    _secondLineLabel.shadowColor = [UIColor blackColor];
    _secondLineLabel.shadowOffset = CGSizeMake(0, 2);
    _secondLineLabel.font = [UIFont systemFontOfSize:16.0f];
    
    [_showView addSubview:self.avatarButton];
    [_showView addSubview:self.firstLineTextLabel];
    [_showView addSubview:self.secondLineLabel];
...
- (void)dealloc {
    ...
    self.firstLineTextLabel = nil;
    self.secondLineLabel = nil;
